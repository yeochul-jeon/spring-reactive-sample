...
Generate a new project at [Spring Initializr](https://start.spring.io/) with the following settings:

- **Project**: Maven
- **Language**: Java
- **Spring Boot**: 4.x
- **Java**: 25
- **Dependencies**: `Cassandra`, `Webflux`, `Lombok`, `Testcontainers`

The `spring-boot-starter-cassandra` starter auto-configures the connection pool and exposes the `CqlSession` bean. Since Cassandra's DSE driver includes native reactive support, no additional reactive driver is needed. 
...
- **Spring Boot**: 4.x
- **Java**: 25
- **Dependencies**: `Couchbase`, `Webflux`, `Lombok`, `Testcontainers`

The `spring-boot-starter-couchbase` starter configures the SDK and bootstraps the `Cluster` connection. The Couchbase Java SDK exposes a `ReactiveCollection` API natively, so no additional reactive driver is needed — just call `.reactive()` on any `Collection` reference.
...
Create a new project at [Spring Initializr](https://start.spring.io/) with:

- **Project**: Maven
- **Language**: Java
- **Spring Boot**: 4.x
- **Java**: 25
- **Dependencies**: `Spring Data Elasticsearch`, `Webflux`, `Lombok`, `Testcontainers`

> [!IMPORTANT]
> Unlike the other stores in this guide, Elasticsearch is an exception: the official Elasticsearch Java Client SDK does **not** include a reactive client. Spring Data Elasticsearch fills this gap by providing its own `ReactiveElasticsearchClient` wrapper built on top of the low-level REST client. As a consequence, you cannot use a lightweight `spring-boot-starter-elasticsearch` alone — you must pull in the full Spring Data Elasticsearch starter to access the reactive client.
...
Head to [Spring Initializr](https://start.spring.io/) and generate a project with:

- **Project**: Maven
- **Language**: Java
- **Spring Boot**: 4.x
- **Java**: 25
- **Dependencies**: `MongoDB`, `Webflux`, `Lombok`, `Testcontainers`

The `spring-boot-starter-mongodb` starter exposes both the synchronous and reactive `MongoClient` beans. When used in a reactive application, the starter registers a `com.mongodb.reactivestreams.client.MongoClient` bean automatically.

> [!TIP]
> The starter transitively includes the synchronous `mongodb-driver-sync` driver even when you only need the reactive driver. For a leaner classpath, explicitly exclude `mongodb-driver-sync` and declare `mongodb-driver-reactivestreams` directly, as shown below:
...
Open [Spring Initializr](https://start.spring.io/) and create a project with:

- **Project**: Maven
- **Language**: Java
- **Spring Boot**: 4.x
- **Java**: 25
- **Dependencies**: `Neo4j`, `Webflux`, `Lombok`, `Testcontainers`

The `spring-boot-starter-neo4j` starter bootstraps the Neo4j `Driver` which supports synchronous, async, and reactive sessions out of the box. No additional reactive driver artifact is required — simply request a `ReactiveSession` from the already-configured `Driver` bean at runtime.
